...
fn test_weighted_path_multiple_options() {
    ...
    for id in [2, 3, 4, 5] {
        client.known_nodes.lock().unwrap().insert(id, NodeType::Drone);
    }
    client.known_nodes.lock().unwrap().insert(6, NodeType::Server);
...
fn test_weighted_path_congested_network() {
    ...
    client.known_nodes.lock().unwrap().insert(3, NodeType::Drone);
    client.known_nodes.lock().unwrap().insert(4, NodeType::Server);
...
fn test_weighted_path_mesh_network() {
    ...
    client.known_nodes.lock().unwrap().insert(8, NodeType::Server);
...
fn test_weighted_path_redundant_layers() {
    ...
    for id in 8..=10 {
        client.known_nodes.lock().unwrap().insert(id, NodeType::Server);
    }
...
fn create_test_client() -> RustbustersClient {
    ...
    RustbustersClient::new(1, tx_ctrl, rx_ctrl, rx_packet, HashMap::new(), None)
}
